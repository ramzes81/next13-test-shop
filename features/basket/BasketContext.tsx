...
export const BasketContext = React.createContext<BasketContextValue | undefined>(undefined);

export default function BasketContextProvider({children}) {
    const [basket = {}, setBasket] = usePersistedState<BasketState>({}, { name: "basket" });
    ...
    const totalQuantity = useMemo(() => {
        return Object.entries(basket).reduce((result, [,product]) => result + product.quantity, 0);
    }, [basket]);

    const totalSum = useMemo(() => {
        return Object.entries(basket).reduce((result, [,product]) => result + product.price * product.quantity, 0);
    }, [basket]);
