...
export default function Checkout() {
    ...
    const {data: validatedPromoCode, error} = useSWR<PromocodeResponse>(appliedPromo ? ['/api/promocode', appliedPromo]: null,
        (url, appliedPromo) =>
    ...
    const totalSumWithPromo = useMemo(() => validatedPromoCode
    ...
    const {data: checkOutResult, error: checkOutError} = useSWR<CheckoutResponse>(checkOutTimestamp ? ['/api/checkout', checkOutTimestamp]: null,
        (url) =>
            fetch<CheckoutResponse>(url, {
                ...
                method: 'POST',
                body: JSON.stringify({basket: Object.values(basket), cardNumber: creditCardInput})
            }),
        {revalidateOnFocus: false}
    );
    ...
    return <div>
        <h1>Checkout</h1>
        {Object.entries(basket || {}).map(([,product]) => <ProductStyles key={product.sku}>
        ...
        <div>
            <label>
                Enter Promo Code
                <input type="text" value={promoUserInput} onChange={(e) => setUserPromoInput(e.target.value)}/>
            </label>
            <input type="button" value="Apply Promo Code" onClick={() => setAppliedPromo(promoUserInput)}/>
        </div>
        <div>
            Sub Total : {totalSum?.toFixed(2)}
        </div>
        {validatedPromoCode && <div>Promotional Discount Amount: {validatedPromoCode.amount}</div>}
        <div>
            Basket Total: {totalSumWithPromo?.toFixed(2)}
        </div>
        <div>
            <label>
                Please enter your credit card number
                <input type="text" value={creditCardInput} onChange={(e) => setCreditCardInput(e.target.value)}/>
            </label>
            <input type="button" value="Checkout" onClick={() => setCheckOutTimestamp(new Date().getTime())}/>
        </div>
    </div>;
}